...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM accounts WHERE username = ? AND password = ? AND admin = 1', (username, password))
            admin_account = cursor.fetchone()
            conn.close()
            if admin_account:
                return render_template('admin.html', msg=msg)
            else:
                return render_template('regular.html', msg=msg)
...
def regularlanding():
    return render_template('regular.html') #landing page for non-admin users
